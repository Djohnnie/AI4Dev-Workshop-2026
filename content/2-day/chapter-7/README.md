[🏠 Workshop Home](../../../README.md) | [📊 Slides](SLIDES.md) | [← Chapter 6 — AI Across the Entire Software Lifecycle](../chapter-6/README.md) | [Chapter 8 — Get Your Hands Dirty: Real-World AI in Action →](../chapter-8/README.md)
...
- GitHub Docs best-practice recap: the habits that consistently improve output quality
  - **Choose the right tool for the job:** use inline suggestions for local, predictable code; use Chat for natural-language questions, larger generation tasks, and iterative refinement; use agentic flows when the task needs verification, multiple files, or tool use
  - **Prompt with structure:** break large tasks into smaller steps and give Copilot four things whenever possible — the task, the surrounding context, an example, and the constraints that define "done"
  - **Guide the context window:** open relevant files, close irrelevant tabs, remove stale chat context when it is hurting the conversation, and start a fresh thread when the task changes materially
  - **Stay accountable for the output:** understand the code before accepting it, review for security/readability/maintainability, and use tests plus automated tooling as the backstop rather than trusting polished-looking code
  - **Use the feedback loop:** if the first answer is close but not right, rephrase the prompt, ask for alternatives, compare suggestions, and use accept/reject or thumbs-up/down feedback to steer future output
- Copilot CLI tips and tricks from the docs
  - **Customize first:** keep repository instructions concise and actionable, and remember that repository instructions override global ones; this is the fastest way to encode your team's build, test, and style expectations
  - **Control tool permissions deliberately:** pre-allow safe command families when useful, reset approved tools when you want a clean slate, and avoid granting broad write access unless the session genuinely needs it
  - **Pick the model for the task:** use the default/auto path for most work, switch to a deeper reasoning model for architecture or tricky debugging, and use faster code-oriented models for routine implementation and review
  - **Use plan mode for larger changes:** `/plan` (or plan mode) improves success on multi-file tasks by forcing clarification, sequencing, and approval before code is written
  - **Keep sessions focused:** infinite sessions remove hard context limits, but `/clear` or `/new` still improves quality between unrelated tasks; `/context` and `/session` help you inspect what the CLI is carrying
  - **Delegate tangential work:** `/delegate` is strongest for long-running or asynchronous work you do not need to babysit locally; keep interactive debugging and core feature work in the local session
- Practical tips and tricks from the `Cafe_Final` case study
  - **Locate before you ask:** use free tools first — IDE search, go-to-definition, scoped folder listing, `dotnet sln list`, ripgrep — then give Copilot only the exact file, symbol, or snippet that matters; ask AI for search terms or likely project names, not for a blind repo-wide expedition
  - **Conversation hygiene saves both cost and quality:** start a new chat for a new topic, summarise or compact long threads before they bloat, replace stale context instead of stacking revised versions, and harvest conclusions into docs/instructions before restarting fresh
  - **Scope references aggressively:** a selected method or symbol beats a full-file reference, and a single file beats `@workspace`; `#file:` / attached files are convenient but still pull full content into context, so use selection- or symbol-level scope whenever possible
...
- Prompt files, skill files, and custom agent files: codifying your team's AI patterns as version-controlled knowledge
  ...
  - **Build context for the team, not for one hero user:** instruction files, skill files, prompt files, and agent files should capture shared ways of working so that any developer can benefit from the same high-quality context without relying on tribal knowledge or one person's private prompt habits
  ...
  - **Documentation matters even more in an AI-assisted team:** the better your docs explain architecture, rules, and "what not to do," the easier it is to encode that guidance into reusable context files and keep the whole team aligned while they experiment and learn together
...
[🏠 Workshop Home](../../../README.md) | [← Chapter 6 — AI Across the Entire Software Lifecycle](../chapter-6/README.md) | [Chapter 8 — Get Your Hands Dirty: Real-World AI in Action →](../chapter-8/README.md)